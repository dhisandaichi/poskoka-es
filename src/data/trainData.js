export const stationConfig = {
    KAC: { code: "KAC", name: "KIARACONDONG", totalTracks: 6, rules: { boardingOpen: 60, stopCheck: 10, boardingClose: 5 } },
    BD: { code: "BD", name: "BANDUNG", totalTracks: 7, rules: { feederCheck: 5, boardingOpen: 60, stopCheck: 20, boardingClose: 5 } }
};
...
export const trainSchedule = [
    ...
    { station: 'BD', no: 'KA 366', name: 'Commuter Line Bandung Raya', route: 'PADALARANG - CICALENGKA', arr: '19:25', dep: '19:27', track: 7, type: 'LOC' },
...
// Utility functions
export function getMinutesDifference(targetTimeStr, currentTimeObj) {
    ...
    return Math.floor((targetDate - currentTimeObj) / 60000);
}

export function parseRoute(routeStr) {
    const cleanRoute = routeStr.replace(/->/g, '-');
    const parts = cleanRoute.split('-').map(s => s.trim());
    return parts.length >= 2
        ? { origin: parts[0], destination: parts[parts.length - 1] }
        : { origin: routeStr, destination: routeStr };
}

export function getProcessedSchedule(stationCode, currentTime) {
    ...
    const result = stationTrains.map(train => {
        ...
        let displayStatus = '', displayInfo = '', isBlinking = false, mainTime = '';
        ...
        if (destination.toUpperCase().includes(stationName) || !train.dep) {
            ...
            if (minToDep !== null && minToDep > 0 && minToDep <= 180) {
                displayStatus = `Berangkat ${minToDep} mnt lagi`;
                displayInfo = `Tujuan ${destination}`;
                mainTime = train.dep;
            ...
            if (minToArr !== null && minToArr > 0 && minToArr <= stationConfig[stationCode].rules.stopCheck) {
                ...
                mainTime = train.arr;
                if (minToArr < 5) isBlinking = true;
            } else if (minToDep !== null && minToDep > 0 && minToDep <= 180) {
                displayStatus = `Berangkat ${train.dep}`;
                mainTime = train.dep;
                if (minToDep < 5) isBlinking = true;
            }
            displayInfo = `Tujuan ${destination} · Dari ${origin}`;
        }

        return {
            raw: train, track: train.track, status: displayStatus, info: displayInfo,
            time: mainTime, isBlinking,
            displayMode: train.type === 'LOC' ? 'KCI' : train.type === 'FDR' ? 'KCIC' : 'KAI',
            minToArr, minToDep,
            styles: {
                kci: { line1: destination, line2: `Via ${origin} · ${train.no}`, status: displayStatus, eta: mainTime },
                kai: { trainName: `${train.name} (${train.no})`, destination, time: mainTime, platform: train.track, info: displayInfo }
            }
        };
    });
    return result.filter(item => item.status !== '');
}
